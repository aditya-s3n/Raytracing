...
inline vec3 random_in_unit_disk() {
    while (true) {
        auto p = vec3(random_double_vec(-1, 1), random_double_vec(-1, 1), 0);
        
        if (p.lenght_sqr() < 1) return p;
    }
}
...
inline vec3 refract(const vec3& uv, const vec3& n, double etai_over_etat) {
    auto cost_theta = std::fmin(dot(uv * -1, n), 1.0);
    vec3 r_out_perp = etai_over_etat * (uv + cost_theta*n);
    vec3 r_out_parallel = -std::sqrt(std::fabs(1.0 - r_out_perp.lenght_sqr())) * n;
